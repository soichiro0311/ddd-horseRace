...
describe("馬券", () => {
  ...
  describe("様式と登録した予想着順のバリデーション", () => {
    describe("単勝", () => {
      test("様式が単勝 かつ 2着予想の馬を登録していた場合、馬券を購入できないこと", () => {
        const race = new Race(1, "大井競馬場", new Date("2025/1/3 14:00:00"));

        expect(
          () => new RacingTicket(race, new Tansho(), 100, "HORSE1", "HORSE2")
        ).toThrowError(
          new DomainError(
            `単勝の場合は1着予想の馬のみを入力してください。レース番号: ${race.raceNumber()}, 競馬場: ${race.trackName()}, 1着予想: HORSE1, 2着予想: HORSE2, 3着予想: undefined`
          )
        );
      });
      test("様式が単勝 かつ 3着予想の馬を登録していた場合、馬券を購入できないこと", () => {
        const race = new Race(1, "大井競馬場", new Date("2025/1/3 14:00:00"));
        ...
        expect(
          ...
          new DomainError(
            `単勝の場合は1着予想の馬のみを入力してください。レース番号: ${race.raceNumber()}, 競馬場: ${race.trackName()}, 1着予想: HORSE1, 2着予想: HORSE2, 3着予想: HORSE3`
          )
        );
      });
      test("様式が単勝 かつ 1着予想の馬を登録していない場合、馬券を購入できないこと", () => {
        const race = new Race(1, "大井競馬場", new Date("2025/1/3 14:00:00"));
        ...
        expect(
          ...
          new DomainError(
            `単勝の場合は1着予想の馬のみを入力してください。レース番号: ${race.raceNumber()}, 競馬場: ${race.trackName()}, 1着予想: undefined, 2着予想: undefined, 3着予想: HORSE1`
          )
        );
      });
    });
